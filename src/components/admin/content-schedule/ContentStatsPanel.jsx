            ...
            return (
              <div key={item.id} className="flex items-center gap-3 rounded-xl p-2.5" style={{ background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
                <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0">
                  <ContentThumbnail item={{ ...item, unlocked: new Date(item.scheduled_at).getTime() <= Date.now() }} fallback={<div className="w-full h-full flex items-center justify-center text-sm" style={{ background: `${meta.color}15` }}>{meta.emoji}</div>} />
                </div>
