...
import { FileText, Search, Heart, MessageCircle } from "lucide-react";
...
export default function InstitutionDropsTab({ ownerEmail }) {
  ...
  const { data: drops = [], isLoading: dropsLoading } = useQuery({
    ...
    queryFn: () => base44.entities.GlowDrop.list("-created_date", 1000), // Get recent drops
  });
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full bg-[#121826] border border-white/10 rounded-xl py-2 pl-9 pr-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#00CFFF]/50"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredDrops.length === 0 ? (
          ...
          filteredDrops.map(drop => {
            ...
            return (
              <div key={drop.id} className="bg-[#121826] border border-white/5 rounded-2xl overflow-hidden flex flex-col h-[300px]">
                {drop.media_url ? (
