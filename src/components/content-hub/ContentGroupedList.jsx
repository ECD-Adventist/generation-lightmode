...
export default function ContentGroupedList({ items, locked = false, horizontal = false }) {
  ...
  return <div className="space-y-10">{groups.map(({ type, dates }) => (
    <section key={type.id}>
      <h3 className="glm-headline text-xl text-white flex items-center gap-2 mb-5">
        <type.icon size={18} style={{ color: type.color }} /> {type.label}s
      </h3>
      <div className="space-y-8">{dates.map(({ date, languages }) => (
        <div key={date} className="pl-3 border-l" style={{ borderColor: `${type.color}55` }}>
          ...
          <div className="space-y-6">{languages.map(language => {
            ...
            return <div key={language}><p className="text-[11px] font-black uppercase tracking-widest mb-3" style={{ color: type.color }}>{language}</p>
              {horizontal ? (
                <div className="flex gap-4 overflow-x-auto pb-2 snap-x" style={{ scrollbarWidth: "thin" }}>
                  {matching.map(item => <div key={item.id} className="shrink-0 w-72 snap-start">{locked ? <LockedContentCard item={item} /> : <ContentCard item={item} />}</div>)}
                </div>
              ) : (
