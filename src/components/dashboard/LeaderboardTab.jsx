...
export default function LeaderboardTab({ user }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 font-['Inter']">
      <div className="rounded-[1.75rem] p-8 relative overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid #E0EAF5", boxShadow: "0 8px 32px rgba(11, 63, 217, 0.08)" }}>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 shrink-0">
            <div className="flex p-1.5 rounded-xl shrink-0" style={{ background: "#F6F8FC", border: "1px solid #E0EAF5" }}>
              <button onClick={() => setRegion('global')} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all" style={region === 'global' ? { background: "linear-gradient(90deg, #FFD60A, #FF9F1A)", color: "#0B1B3D", boxShadow: "0 2px 6px rgba(255, 159, 26, 0.25)" } : { background: "transparent", color: "#6B7FA0" }}>Global</button>
              <button onClick={() => setRegion('regional')} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all" style={region === 'regional' ? { background: "linear-gradient(90deg, #FFD60A, #FF9F1A)", color: "#0B1B3D", boxShadow: "0 2px 6px rgba(255, 159, 26, 0.25)" } : { background: "transparent", color: "#6B7FA0" }}>Regional</button>
            </div>
            <div className="flex p-1.5 rounded-xl shrink-0 text-sm items-center px-4" style={{ background: "#F6F8FC", border: "1px solid #E0EAF5", color: "#6B7FA0" }}>
        ...
        <div className="overflow-x-auto relative z-10">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-2 text-xs font-bold uppercase tracking-wider" style={{ borderColor: "#E0EAF5", color: "#6B7FA0" }}>
                    ...
                    <tr key={u.id} className="border-b transition-all duration-300" style={{ borderColor: "#F0F4FA", background: isMe ? "#EEF3FF" : "transparent" }}
                      ...
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold overflow-hidden" style={{ background: "#F6F8FC", border: "1px solid #E0EAF5", color: "#4A5878" }}>
