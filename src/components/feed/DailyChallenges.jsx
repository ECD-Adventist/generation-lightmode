...
export default function DailyChallenges({ user }) {
  ...
  return (
    <div className="rounded-[24px] p-5 mb-6 relative overflow-hidden" style={{ background: "#FFFDF5", border: "1px solid #FFE082", boxShadow: "0 4px 16px rgba(255, 208, 0, 0.05)" }}>
      <div className="flex items-center gap-2 mb-4">
        <Target className="w-4 h-4" style={{ color: "#8B6914" }} />
        <h3 className="font-black text-[11px] tracking-widest uppercase" style={{ color: "#0B1B3D" }}>Daily Missions</h3>
      </div>
        ...
        {challenges.map(challenge => {
          ...
          return (
            <div key={challenge.id} className="p-3 rounded-xl transition-all" style={isCompleted
              ...
              : { background: "#FFFFFF", border: "1px solid transparent", boxShadow: "0 2px 6px rgba(0,0,0,0.04)" }}>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-[10px] mt-0.5" style={{ color: "#8A97B5" }}>{challenge.description}</p>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full" style={{ background: "#FFB400", color: "#0A1A3D" }}>+{challenge.xp} XP</span>
                  {isCompleted ? (
                    <CheckCircle2 className="w-4 h-4 text-green-500" />
                  ) : (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ background: "#F5F5F5", color: "#8A97B5" }}>
                      PENDING
                    </span>
                  )}
                </div>
